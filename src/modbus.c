...
static unsigned int compute_response_length_from_request(modbus_t *ctx, uint8_t *req)
{
    ...
    case MODBUS_FC_READ_STRING:
        /* For slave_id:  The response is device specific (the header provides the
           length)
           For read_string: The response depends on the server */
                                                  ...
                                                  msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION)
    {
        if (function <= MODBUS_FC_WRITE_SINGLE_REGISTER ||
            function == MODBUS_FC_READ_STRING ||
        ...
        else
        {
            ...
            MODBUS_FC_REPORT_SLAVE_ID, MODBUS_FC_READ_STRING */
            length = 0;
        }
    }
    else
...
/**
 * If the request and response nb's match, returns nb
 * else returns -1
 * */
                              ...
                              uint8_t *rsp, int rsp_length)
{
        ...
        function < 0x80)
    {
        ...
        case MODBUS_FC_READ_STRING:
             ...
             * or report length of string received
             * */
 ...
 * */
int modbus_reply(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
 ...
 * accordingly.
 * */
int modbus_process_request(modbus_t *ctx, const uint8_t *req,
                           ...
                           modbus_mapping_t *mb_mapping)
{
    ...
    *rsp_length = 0;
    ...
    case MODBUS_FC_READ_STRING:
    {
        /* nb stores the number of characters in the token */
        int nb = strlen((const char *)(mb_mapping->tab_string));

        if (nb < 1 || nb > MODBUS_MAX_STRING_LENGTH)
        {
            *rsp_length = response_exception(
                ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, TRUE,
                "Illegal token size: %d < 0 or %d > %d\n", nb, nb, MODBUS_MAX_STRING_LENGTH);
        }
        else
        {
            int i;
            ...
            *rsp_length = ctx->backend->build_response_basis(&sft, rsp);
            ...
            for (i = 0; i < nb; i++)
            {
                rsp[(*rsp_length)++] = mb_mapping->tab_string[i];
            }
        }
    }
    break;
    case MODBUS_FC_WRITE_STRING:
    {
        ...
        else
        {
            ...
            {
                /* 6 = first value */
                mb_mapping->tab_string[i] = req[offset + 5 + i];
            }

            *rsp_length = ctx->backend->build_response_basis(&sft, rsp);
            /* 4 bytes: copy 2 empty bytes (unused address) and 2 bytes with the size of the string */
            memcpy(rsp + *rsp_length, req + *rsp_length, 4);
            *rsp_length += 4;
        }
    }
...
static int read_io_status_build_request(modbus_t *ctx, int function,
                                        int addr, int nb, uint8_t *req)
{
    return ctx->backend->build_request_basis(ctx, function, addr, nb, req);
}
                          ...
                          int addr, int nb, uint8_t *dest)
{
    ...
    req_length = read_io_status_build_request(ctx, function, addr, nb, req);
...
/* Constructs a request to read the boolean status of bits */
int modbus_read_bits_build_request(modbus_t *ctx, int addr, int nb, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return read_io_status_build_request(ctx, MODBUS_FC_READ_COILS,
                                        addr, nb, req);
}
...
/* Constructs a request to read the remote device input table */
int modbus_read_input_bits_build_request(modbus_t *ctx, int addr, int nb, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return read_io_status_build_request(ctx, MODBUS_FC_READ_DISCRETE_INPUTS,
                                        addr, nb, req);
}
...
/* Constructs a request to read input and holding registers */
static int read_registers_build_request(modbus_t *ctx, int function, int addr,
                                        int nb, uint8_t *req)
{
                          ...
                          uint16_t *dest)
{
    ...
    req_length = read_registers_build_request(ctx, function, addr, nb, req);
    if (req_length == -1)
    {
        return -1;
    }
...
/* Constructs a request to read holding registers */
int modbus_read_registers_build_request(modbus_t *ctx, int addr, int nb, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return read_registers_build_request(ctx, MODBUS_FC_READ_HOLDING_REGISTERS,
                                        addr, nb, req);
}
...
/* Constructs a request to read input registers */
int modbus_read_input_registers_build_request(modbus_t *ctx, int addr, int nb, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return read_registers_build_request(ctx, MODBUS_FC_READ_INPUT_REGISTERS,
                                        addr, nb, req);
}
...
/* Construct a request to write a single value to bit or register */
static int write_single_build_request(modbus_t *ctx, int function, int addr,
                                      const uint16_t value, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return ctx->backend->build_request_basis(ctx, function, addr, (int)value, req);
}
...
static int write_single(modbus_t *ctx, int function, int addr, const uint16_t value)
{
    ...
    req_length = write_single_build_request(ctx, function, addr, (int)value, req);
    if (req_length == -1)
    {
        return -1;
    }
...
/* Construct a request to write a single bit value */
int modbus_write_bit_build_request(modbus_t *ctx, int addr, int status, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return write_single_build_request(ctx, MODBUS_FC_WRITE_SINGLE_COIL, addr,
                                      status ? 0xFF00 : 0, req);
}
...
/* Construct a request to write a single register */
int modbus_write_register_build_request(modbus_t *ctx, int addr, int value, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return write_single_build_request(ctx, MODBUS_FC_WRITE_SINGLE_REGISTER, addr,
                                      value, req);
}
...
/* Construct a request to write multiple bits */
int modbus_write_bits_build_request(modbus_t *ctx, int addr, int nb, const uint8_t *src, uint8_t *req)
{
    int byte_count;
    int req_length;
    int bit_check = 0;
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    req_length = modbus_write_bits_build_request(ctx, addr, nb, src, req);
    if (req_length == -1)
    {
        return -1;
    }
...
/* Construct a request to write multiple registers */
int modbus_write_registers_build_request(modbus_t *ctx, int addr, int nb, const uint16_t *src, uint8_t *req)
{
    int req_length;
    int byte_count;
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    ...
    req_length = modbus_write_registers_build_request(ctx, addr, nb, src, req);
    if (req_length == -1)
    {
        return -1;
    }
...
/* Construct a request to read a string */
int modbus_read_string_build_request(modbus_t *ctx, uint8_t *req)
{
    if (ctx == NULL)
    {
        errno = EINVAL;
    ...
    return ctx->backend->build_request_basis(ctx, MODBUS_FC_READ_STRING, 0x0, 0x0, req);
}

/* Read a string from the remote device */
int modbus_read_string(modbus_t *ctx, uint8_t *dest)
{
    ...
    req_length = modbus_read_string_build_request(ctx, req);
    if (req_length == -1)
    {
        return -1;
    }
...
/* Construct a request to write a string */
int modbus_write_string_build_request(modbus_t *ctx, const uint8_t *str,
                                      int str_length, uint8_t *req)
{
    int req_length;
    int byte_count;
    int addr;
...
int modbus_write_string(modbus_t *ctx, const uint8_t *str, int str_length)
{
    ...
    req_length = modbus_write_string_build_request(ctx, str, str_length, req);
    if (req_length == -1)
    {
        return -1;
    }
...
int modbus_mask_write_register_build_request(modbus_t *ctx, int addr, uint16_t and_mask, uint16_t or_mask, uint8_t *req)
{
    int req_length;
...
int modbus_mask_write_register(modbus_t *ctx, int addr, uint16_t and_mask, uint16_t or_mask)
{
    ...
    req_length = modbus_mask_write_register_build_request(ctx, addr, and_mask, or_mask, req);
...
/* Construct a request to write and read multiple registers. */
int modbus_write_and_read_registers_build_request(modbus_t *ctx,
                                                  int write_addr, int write_nb,
                                                  const uint16_t *src, int read_addr,
                                                  int read_nb, uint8_t *req)
{
    int rc;
    int req_length;
    int byte_count;
...

{
    ...
    req_length = modbus_write_and_read_registers_build_request(ctx, write_addr, write_nb, src,
                                                               read_addr, read_nb, req);
    if(req_length == -1)
    {
        return -1;
...
/* Construct a request to get the slave ID of the device */
int modbus_report_slave_id_build_request(modbus_t *ctx, int max_dest, uint8_t *req)
{
    int req_length;
...
int modbus_report_slave_id(modbus_t *ctx, int max_dest, uint8_t *dest)
{
    ...
    req_length = modbus_report_slave_id_build_request(ctx, max_dest, req);
    if(req_length == -1)
    {
        return -1;
    }
 ...
 * ENOMEM.
 *
 * MODIFIED: Added tab_stringthis function
 **/
